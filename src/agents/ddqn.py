...
class DDQNAgent:
    """ Implements a an agent that learns using a DQN which is updated
    ...
    def update(self) -> None:
        ...
        # Compute the state action values of the next states Q(s_{t+1}, a) 
        # using the policy network and then select the actions  
        # that maximize this value for each state 
        with torch.no_grad():
            a_max = self.policy_net(non_final_next_states).argmax(dim=1).unsqueeze(1)

        # Compute the state values V(s_{t+1}) for all next states 
        # using the double Q-learning algorithm
        next_state_values = torch.zeros(self.batch_size)
        with torch.no_grad():
            next_state_values[non_final_mask] = self.target_net(non_final_next_states).gather(1, a_max).flatten()
...
if __name__ == "__main__":
    ...
    plot_single_episode(algorithm = "DDQN", episode_durations = seed_episode_durations, num_episodes = num_episodes)

    # Plot the performance of the algorithm over the seeds
    plot_multiple_episodes(algorithm = "DDQN", episode_durations_over_seeds = episode_durations_over_seeds)
